...
func (ctx *httpContext) OnHttpRequestHeaders(int, bool) types.Action {
	...
	org := "foo"
	tenant := "bar"
	org_id, _, err := proxywasm.GetSharedData("o_" + org)
	if err != nil {
		// Cache miss
	}
	tenant_id, _, err := proxywasm.GetSharedData("t_" + string(org_id) + "_" + tenant)
	if err != nil {
		// Cache miss
	}

	// TODO: actually format the IDs
	proxywasm.AddHttpRequestHeader("org_id", string(org_id))
	proxywasm.AddHttpRequestHeader("tenant_id", string(tenant_id))
